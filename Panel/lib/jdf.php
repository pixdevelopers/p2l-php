...
version 2.45 :: 1390/06/27 = 1432/10/19 = 2011/09/18 */
...
function jdate($format,$timestamp='',$none='',$time_zone='Asia/Tehran',$tr_num='fa'){
    ...
    if($time_zone!='local'){
        if($time_zone=='')$time_zone='Asia/Tehran';
    ...
    for($i=0; $i<strlen($format); $i++){
        ...
        switch($sub){
            ...
            case'd':
                $out.=str_pad($j_d,2,0,STR_PAD_LEFT);
                break;
            ...
            case'k';
                $out.=tr_num(100-round(($doy/($kab+365)*100),1),$tr_num);
                break;

            case'K':
                $out.=tr_num(round(($doy/($kab+365)*100),1),$tr_num);
                break;
            ...
            case'm':
                $out.=str_pad($j_m,2,0,STR_PAD_LEFT);
                break;
            ...
            case'w':
                $out.=($date[7]!=6)?$date[7]+1:0;
                break;

            case'W':
                $avs=(($date[7]!=6)?$date[7]+1:0)-($doy%7);
                if($avs<0)$avs+=7;
                $num=(int)(($doy+$avs)/7);
                if($avs<4){
                    $num++;
                ...
                $aks=$avs+$kab;
                if($aks==7)$aks=0;
                $out.=($aks<3 and ($kab+363-$doy)<$aks)?'01':str_pad($num,2,0,STR_PAD_LEFT);
                break;
...
function jstrftime($format,$timestamp='',$none='',$time_zone='Asia/Tehran',$tr_num='fa'){
    ...
    for($i=0; $i<strlen($format); $i++){
        ...
        switch($sub){
            ...
            case'e':/* ! */
                $out.=str_pad($j_d,2,' ',STR_PAD_LEFT);
                break;
            ...
            case'U':
                ...
                $out.=str_pad($num,2,0,STR_PAD_LEFT);
                break;

            case'V':/* ! */
                $avs=(($date[6]!=6)?$date[6]+1:0)-($doy%7);
                if($avs<0)$avs+=7;
                $num=(int)(($doy+$avs)/7);
                if($avs<4){
                    $num++;
            ...
            case'g':/* ! */
                $jdw=($date[6]!=6)?$date[6]+1:0;
                $dny=364+$kab-$doy;
                $out.=substr(($doy<3 and $jdw>($doy+3))?$j_y-1:(($dny<3 and (3-$dny)>$jdw)?$j_y+1:$j_y),2,2);
                break;
            ...
            case'l':/* ! */
                $out.=str_pad((int)$date[0],2,' ',STR_PAD_LEFT);
                break;
            ...
            case'D':/* ! */
                $out.=substr($j_y,2,2).'/'.str_pad($j_m,2,0,STR_PAD_LEFT).'/'.str_pad($j_d,2,0,STR_PAD_LEFT);
                break;

            case'F':/* ! */
                $out.=$j_y.'-'.str_pad($j_m,2,0,STR_PAD_LEFT).'-'.str_pad($j_d,2,0,STR_PAD_LEFT);
                break;
    ...
    foreach($array as $type => $num){
        ...
        switch($type){
            ...
            case'ss':
                ...
                $array[$type]=(($num>99)
                        ?str_ireplace(array('20','13','14'),array('دوهزار','هزار و سیصد','هزار و چهارصد'),substr($num,0,2))
                        .((substr($num,2,2)=='00')?'':' و '):'')
                    .$h3.$p34.$h34.$h4;
                break;
            ...
            case'mm':
                $key=
                    array('فروردین','اردیبهشت','خرداد','تیر','مرداد','شهریور','مهر','آبان','آذر','دی','بهمن','اسفند');
...
Copyright(C)2011, Reza Gholampanahi [ http://jdf.scr.ir/jdf ] version 2.40 */
...
function jalali_to_gregorian($j_y,$j_m,$j_d,$mod=''){
    ...
    if($doy_j>$d28x){
        $gy=$j_y+622; $gd=$doy_j-$d28x;
    }else{
        $gy=$j_y+621; $gd=$doy_j+79;
    }
...
/* [ jdf.php ] version 2.45 ?> Download new version from [ http://jdf.scr.ir ] */