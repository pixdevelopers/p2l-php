...
elseif($General->CheckPrivilege('Package_Manage') == '0'){
    $show_page = '0';
    $error = 'Your account doesn\'t have sufficient privilege to this part! ';
}
elseif(isset($_GET['ID'])) {
    $info = $Package->SelectPackage('1','',$_GET['ID'],'','','','','','1');
    if(mysqli_num_rows($info)>0){
        $row_info = mysqli_fetch_assoc($info);
    }
    else{
        $show_page = '0';;
        $error = 'System can\'t find any result for your request.';
    }
}
else {
    $show_page = '0';
                    ...
                    <div class="col-sm-12">
                        <div class="panel panel-flat" style="padding-top: 15px;">
                            <?php if($show_page == '1') { ?>
                                ...
                                <form action="Package_action.php?action=edit&ID=<?php echo $row_info['ID']; ?>" method="post" target="_blank">
                                    <div class="col-sm-12">
                                        <div class="col-sm-3">
                                            <label class="control-label">Name</label>
                                            <div style="clear: both"></div>
                                            <input type="text" name="Name" value="<?php echo $row_info['Name']; ?>">
                                        </div>
                                        <div class="col-sm-3">
                                            <label class="control-label">Price</label>
                                            <div style="clear: both"></div>
                                            <input type="text" name="Price" value="<?php echo $row_info['Price']; ?>">
                                        </div>
                                        <!--<div class="col-sm-3">
                                            <input type="checkbox" name="IsSpecial" value="1" <?php //if($row_info['IsSpecial'] == '1'){ ?> checked <?php //} ?> >
                                            Special package
                                        </div>-->
                                        <div class="col-sm-3">
                                            <label class="control-label">Enable</label>
                                            <div style="clear: both"></div>
                                            <input type="checkbox" name="Status" value="1" <?php if($row_info['Status']== '1'){ ?> checked <?php } ?> >
                                        </div>
                                    </div>
                                    <div class="col-sm-12">
                                        <?php
                                        $list_features = $Features->Select('','1','Name','ASC','');
                                        $option_count = mysqli_num_rows($list_features);
                                        $i = '1';
                                        $last_break = '';
                                        while($row_list_features = mysqli_fetch_assoc($list_features)){
                                            if(($i%4) == '1') {
                                                $last_break = $i;
                                                ?>
                                                <div class="row">
                                            ...
                                            <div class="col-sm-3">
                                                <input type="checkbox" name="Features[]" value="<?php echo $row_list_features['ID']; ?>" <?php if($Package->CheckPackageAndFeatureRelation($row_info['ID'],$row_list_features['ID']) != '0'){ ?> checked <?php } ?> >
                                                 <?php echo $row_list_features['Name']; ?>
                                            </div>
                                            <?php

                                            if($i == $option_count || ( ($i%4) == '0') ){
                                                ?>
                                                </div>
                                                <?php
                                            }
